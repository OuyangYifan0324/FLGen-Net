...
class BaseModel():
    ...
    def print_network(self, net):
        ...
        net_str = str(net)
        net_params = sum(map(lambda x: x.numel(), net.parameters()))  # 网络里面的所有参数都统计在内

        from thop import profile, clever_format    # znn
        dummy_input = torch.rand((1, 3, 256, 256)).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))# znn
        flops, net_params = profile(net, inputs=(dummy_input,), verbose=False)  # znn 网络里面的参与推理的所有参数
        flops_str, params_str = clever_format([flops, net_params], "%.2f") # znn
        logger.info( f'Network: {net_cls_str}, with parameters: {params_str} and FLOPS(1*3*256*256): {flops_str}') # znn
        # logger.info(f'Network: {net_cls_str}, with parameters: {net_params:,d}')   znn delete
        logger.info(net_str)
