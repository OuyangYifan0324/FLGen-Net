...
def set_random_seed(seed):
    ...
    torch.manual_seed(seed) # 设置 PyTorch 的随机数生成器的种子。这确保了在 PyTorch 中生成的随机张量（如权重初始化、随机噪声等）是可重复的
    torch.cuda.manual_seed(seed) # 设置当前 GPU 的随机种子
    torch.cuda.manual_seed_all(seed) # 设置所有可用 GPU 的随机种子。这在多 GPU 环境中非常有用，确保每个 GPU 上的随机数生成器行为一致。
